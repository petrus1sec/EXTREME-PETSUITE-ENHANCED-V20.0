#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# PETSUITE PROMAX V15.0 EXTREME EDITION
...
class PayloadDB:
    def __init__(self):
        ...
        self.jwt_weak = [
            ...
            # Common secrets
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c",  # secret = "secret"
...
class ExtremeScanner:
    def __init__(self, target):
        ...
        self.sesi.headers = {
            'User-Agent': 'PetSuiteUltimateProMax/15.0',
            'Accept': '*/*'
        }
        self.config = {
            "threads": 5,
            "timeout": 15,
            "contact_scan_depth": 3
        }
    ...
    def scan_sqli(self):
        ...
        for url in test_urls:
            ...
            for param in params:
                for payload in self.payload.sqli[:15]:  # Gunakan 15 payload pertama untuk demo
                    try:
    ...
    def scan_xss(self):
        ...
        for url in test_urls:
            ...
            for param in params:
                for payload in self.payload.xss[:10]:  # Gunakan 10 payload pertama
                    try:
                        # Buat URL dengan payload
                        temp_params = params.copy()
                        temp_params[param] = [payload]
        ...
        for url in test_urls:
            ...
            for param in params:
                if 'file' in param.lower() or 'page' in param.lower():  # Parameter yang mungkin vulnerable
                    for payload in self.payload.lfi[:5]:  # Gunakan 5 payload pertama
                        try:
                            # Buat URL dengan payload
                            temp_params = params.copy()
                            temp_params[param] = [payload]
    ...
    def scan_rce(self):
        ...
        for url in test_urls:
            ...
            for param in params:
                for payload in self.payload.rce[:5]:  # Gunakan 5 payload pertama
                    try:
                        # Buat URL dengan payload
                        temp_params = params.copy()
                        temp_params[param] = [payload]
        ...
        for url in test_urls:
            ...
            for param in params:
                if 'url' in param.lower() or 'image' in param.lower():  # Parameter yang mungkin vulnerable
                    for payload in self.payload.ssrf[:3]:  # Gunakan 3 payload pertama
                        try:
                            # Buat URL dengan payload
                            temp_params = params.copy()
                            temp_params[param] = [payload]
    ...
    def _get_testable_urls(self):
        ...
        return list(set(urls))[:10]  # Batasi ke 10 URL untuk demo
    
    def _add_vulnerability(self, jenis, parameter, payload, tingkat, solusi):
        ...
        print(f"{Warna.H}[+] {jenis} ditemukan di parameter {parameter}{Warna.RESET}")
    
    def _save_security_report(self):
        """Simpan hasil security scan"""
        report_path = f"{self.report_dir}/extreme_scan.txt"
        
        with open(report_path, 'w') as f:
            f.write("=== HASIL SCAN TADI ANJJ ===\n")
            f.write(f"Target: {self.target}\n")
            f.write(f"Tanggal: {datetime.now()}\n\n")
            
            if not self.temuan:
                f.write("Tidak ditemukan kerentanan keamanan\n")
            else:
                f.write(f"Total kerentanan ditemukan: {len(self.temuan)}\n\n")
                for idx, vuln in enumerate(self.temuan, 1):
                    f.write(f"{idx}. {vuln['jenis']} ({vuln['tingkat']})\n")
                    f.write(f"Parameter: {vuln['parameter']}\n")
                    f.write(f"Payload: {vuln['payload']}\n")
                    f.write(f"Solusi: {vuln['solusi']}\n\n")
        
        print(f"{Warna.H}[+] Hasil security scan tersimpan di: {report_path}{Warna.RESET}")
       ...
       while True:
            ...
            try:
                ...
                if pilihan == "1":
                    ...
                    print(f"\n{Warna.U}=== TENTANG PETSUITE LAST ENHANCED V15.0 ==={Warna.RESET}")
                    print("Tools keamanan web paling lengkap dengan:")
                    print("- 500+ payload kerentanan")
                    print("- Contact grabbing otomatis")
                    print("- Laporan terstruktur")
